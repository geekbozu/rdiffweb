...
CONFIG_KEY = 'rdiffweb_config'


def load_config(filename):
    """
    After calling this method with a configuration file name. Configuration are
    stored in cherrypy config and accessible using Option().
    """
    if not os.path.isfile(filename):
        print("configuration file %s doesn't exists" % filename, file=sys.stderr)
        exit(1)
    try:
        cherrypy.config.update({CONFIG_KEY: read_config(filename)})
    except:
        print("error reading configuration file %s" % filename, file=sys.stderr)
        traceback.print_exc()
        exit(1)
...
class Option(object):
    ...
    def get(self):
        ...
        """
        value = cherrypy.config.get(CONFIG_KEY, {}).get(self.key)
        if value is None:
            return self.default
    ...
    def set(self, value):
        ...
        """
        if value is None:
            cherrypy.config[CONFIG_KEY].set(self.key, None)
        else:
            cherrypy.config.setdefault(CONFIG_KEY, {})[self.key] = self._set_func(value)
